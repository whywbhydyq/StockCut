...
export function exportLinearResultCsv(result: LinearOptimizationResult, unit: DisplayUnit): string {
  return rowsToCsv([
    ['stock', 'stock_label', 'material', 'label', 'instance', 'start', 'length'],
    ...result.stocksUsed.flatMap((stock) => stock.cuts.map((cut) => [stock.stockIndex, stock.stockLabel, stock.material ?? '', cut.partLabel, cut.instanceIndex, formatDimension(cut.startUm, unit), formatDimension(cut.lengthUm, unit)]))
  ]);
}
