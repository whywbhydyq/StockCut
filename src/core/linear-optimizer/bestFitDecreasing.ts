...
interface CutInstance { partId: string; label: string; lengthUm: number; instanceIndex: number; material?: string }
...
function expandCuts(parts: LinearPartInput[], unit: LinearProjectInput['unit']): { cuts: CutInstance[]; warnings: OptimizationWarning[] } {
  ...
  for (const row of parts) {
    ...
    for (let i = 1; i <= quantity.value; i += 1) cuts.push({ partId: row.id, label: row.label || `Cut ${row.id}`, lengthUm: length.valueUm, instanceIndex: i, material: row.material });
  }
...
export function optimizeLinearProject(input: LinearProjectInput): LinearOptimizationResult {
  ...
  for (const cut of orderedCuts) {
    ...
    best.stock.cuts.push({ partId: cut.partId, partLabel: cut.label, instanceIndex: cut.instanceIndex, startUm, lengthUm: cut.lengthUm });
