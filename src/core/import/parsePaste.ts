...
export function parseLinearPaste(text: string): LinearPasteResult {
  ...
  String(text).split(/\r?\n/).filter(Boolean).forEach((row, index) => {
    ...
    records.push({ id: crypto.randomUUID(), label: cells[0], length: cells[1], quantity: cells[2], material: cells[3] || undefined, miterAngle: cells[4] || undefined, notes: cells[5] || undefined });
  });
  return { ok: errors.length === 0, records, errors };
}
