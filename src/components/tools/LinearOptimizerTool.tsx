...
import { validateLinearProjectInput } from '@/core/validation/projectSchema';
...
export function LinearOptimizerTool({ preset = 'imperial-linear' }: { preset?: LinearPresetKey }) {
  ...
  useEffect(() => {
    const shared = readShareProjectFromHash<LinearProjectInput>('linear-1d', validateLinearProjectInput);
    setProject(shared ?? loadProject(`${key}:${preset}`, fallback, validateLinearProjectInput));
  ...
  const importJsonFile = (file?: File) => {
    ...
    void file.text().then((text) => {
      const parsed = validateLinearProjectInput(JSON.parse(text));
  ...
  return (
    <section className="app-workspace" id="linear-tool">
      ...
      <div className="no-print mt-4 flex flex-wrap gap-3"><button className="btn-primary" onClick={addRow}>Add cut</button><button className="btn-primary" onClick={() => { trackEvent('optimize_linear_clicked', { strategy: project.strategy ?? 'least_waste', execution: 'worker' }); setResult(null); setPasteError(null); const job = runLinearOptimizationInWorker(project, setWorkerProgress); cancelWorkerRef.current = job.cancel; setWorkerProgress({ percent: 1, message: 'Starting layout optimization.' }); void job.promise.then((nextResult) => setResult(nextResult)).catch((error: unknown) => setPasteError(error instanceof Error ? error.message : 'Optimization failed.')).finally(() => { cancelWorkerRef.current = null; setWorkerProgress(null); }); }}>Optimize</button><button className="btn-secondary" onClick={() => { trackEvent('sample_loaded', { mode: 'linear', sample: 'imperial-linear' }); setProject(clonePreset(linearPresets['imperial-linear'])); setResult(null); }}>8 ft sample</button><button className="btn-secondary" onClick={() => { trackEvent('sample_loaded', { mode: 'linear', sample: 'metric-linear' }); setProject(clonePreset(linearPresets['metric-linear'])); setResult(null); }}>6 m sample</button>{result && <><button className="btn-secondary" onClick={async () => { trackEvent('csv_export_clicked', { mode: 'linear' }); const { downloadText, exportLinearResultCsv } = await import('@/core/export/exportCsv'); downloadText('stockcut-linear-result.csv', exportLinearResultCsv(result, project.unit), 'text/csv'); }}>CSV export</button><button className="btn-secondary" onClick={async () => { trackEvent('json_export_clicked', { mode: 'linear' }); const { downloadText } = await import('@/core/export/exportCsv'); downloadText('stockcut-linear-project.json', JSON.stringify(project, null, 2), 'application/json'); }}>JSON export</button><button className="btn-secondary" onClick={() => navigator.clipboard.writeText(`StockCut linear: ${result.stocksUsed.length} stocks, waste ${formatPercent(result.wasteRate)}, estimated stock cost ${result.estimatedStockCost ? `$${result.estimatedStockCost.toFixed(2)}` : 'not set'}`)}>Copy summary</button><button className="btn-secondary" onClick={() => { try { navigator.clipboard.writeText(buildShareUrl('linear-1d', project)); trackEvent('share_link_created', { mode: 'linear' }); } catch (error) { setPasteError(error instanceof Error ? error.message : 'Project is too large for a share link. Download JSON instead.'); } }}>Copy share link</button><button className="btn-secondary" onClick={addResultOffcutsToStock}>Use offcuts as stock</button><button className="btn-secondary" onClick={saveResultOffcutsToLibrary}>Save offcuts to library</button><button className="btn-secondary" onClick={async () => { trackEvent('pdf_export_clicked', { mode: 'linear' }); const { downloadLinearPdf } = await import('@/core/export/exportPdf'); await downloadLinearPdf(project, result); }}>Download PDF file</button><button className="btn-secondary" onClick={() => { trackEvent('print_clicked', { mode: 'linear' }); window.print(); }}>Browser Print / Save PDF</button></>}</div>
