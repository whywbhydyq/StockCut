...
function GuideContent({ page }: { page: SeoPage }) {
  const guideBlocks: Record<string, { lead: string; bullets: string[] }> = {
    ...
    '/guillotine-cut-vs-nesting': { lead: 'Guillotine-friendly layouts favor straight cuts across remaining rectangular areas. Industrial nesting may pack parts more tightly but is harder to execute without CNC workflows.', bullets: ['StockCut uses practical rectangular layouts, not industrial nesting guarantees.', 'This MVP intentionally avoids irregular shapes, G-code, and CNC post-processing. The DXF export is a lightweight rectangular planning outline, not a machine-ready toolpath.', 'The result is intended as a shop-readable plan that can be printed and checked.'] },
